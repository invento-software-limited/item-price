...
def item_price_update_or_create(doc,type):
    ...
    if frappe.db.exists("Item Price", {"item_code" : doc.item_code , "{}".format(type) : 1}):
        if doc.get(field_name) != frappe.get_value("Item Price",{"item_code" : doc.get("item_code") , "{}".format(type) : 1},"price_list_rate"):
